...
static void sem_abs_pointers (abs_declr_t * ad, ast_node_t * ast);
...
abs_declr_t *
sem_abstract_declarator (ast_node_t * ast)
{
  assert (ast != NULL);
  assert (IS_ABSTRACT_DECLARATOR (ast->func_ptr));
  assert (ast->children != NULL);

  abs_declr_t *ad = calloc (1, sizeof (abs_declr_t));
  assert (ad != NULL);

  ad->n_pointers = 0;
  ad->pointers = NULL;

  if (ast->func_ptr == abstract_declarator_1)
    {
      sem_abs_pointers (ad, ast);
      /* TODO */
    }
  else if (ast->func_ptr == abstract_declarator_2)
    sem_abs_pointers (ad, ast);
  else if (ast->func_ptr == abstract_declarator_3)
    {
      /* TODO */
    }
...
static void
sem_abs_pointers (abs_declr_t * ad, ast_node_t * ast)
{
  assert (ad != NULL);
  assert (ast != NULL);
  ...
  ad->pointers = calloc (ad->n_pointers, sizeof (pointer_t *));
  assert (ad->pointers != NULL);

  int i = 0;
  for (ptr = ast->children[0]; (child = find_pointer_child (ptr)) != NULL;
       ptr = child, i++)
    {
      ad->pointers[i] = calloc (1, sizeof (pointer_t));
      assert (ad->pointers[i] != NULL);
      ad->pointers[i]->type_qual = find_type_qual_in_pointer (ptr);
    }
}
