...
static int
sec_6_7_1_clause_2 (struct declaration_specifiers *ptr)
{
  assert (ptr != NULL);
  assert (ptr->kind == NODE_DECLARATION_SPECIFIERS);
  ...
  for (struct ds_node * aux = ptr->first; aux != NULL; aux = aux->next)
    if (aux->ds_kind == NODE_STORAGE_CLASS_SPECIFIER)
      switch (aux->stg->value)
        {
        case STG_TYPEDEF:
  ...
  if (masks.n_typedef > 1)
    printf ("[%s]<%s:%d> typedef storage class appears more than once.\n",
            __func__, __FILE__, __LINE__);
  else if (masks.n_extern > 1)
    printf ("[%s]<%s:%d> extern storage class appears more than once.\n",
            __func__, __FILE__, __LINE__);
  else if (masks.n_static > 1)
    printf ("[%s]<%s:%d> static storage class appears more than once.\n",
            __func__, __FILE__, __LINE__);
  else if (masks.n_thread_local > 1)
    printf
      ("[%s]<%s:%d> thread_local storage class appears more than once.\n",
       __func__, __FILE__, __LINE__);
  else if (masks.n_auto > 1)
    printf ("[%s]<%s:%d> auto storage class appears more than once.\n",
            __func__, __FILE__, __LINE__);
  else if (masks.n_register > 1)
    printf ("[%s]<%s:%d> register storage class appears more than once.\n",
            __func__, __FILE__, __LINE__);
  int n_sum =
    (masks.n_auto > 0) + (masks.n_extern > 0) + (masks.n_register >
                                                 0) + (masks.n_static >
                                                       0) +
    (masks.n_thread_local > 0) + (masks.n_typedef > 0);
  if (n_sum > 1)
    {
      ret = -1;
      if (n_sum == 2 && masks.n_thread_local > 0)
        if (masks.n_static > 0 || masks.n_extern > 0)
          ret = 0;
      if (ret == -1)
        printf ("[%s]<%s:%d> Clause 2 of section 6.7.1 has been violated.\n",
                __func__, __FILE__, __LINE__);
    }
